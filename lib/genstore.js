...
export async function uploadDataUrl(dataUrl, prefix = "upload") {
  ...
  if (configured()) {
    try {
      ...
      if (!process.env.FAL_KEY) throw e; // no fal fallback available
    }
  }

  // 2) fal storage fallback (only needs FAL_KEY).
  if (!process.env.FAL_KEY) throw new Error("No file host configured (set BLOB_READ_WRITE_TOKEN or FAL_KEY)");
