...
// https URL. Pass-through for strings that are already http(s). Prefers Vercel
// Blob (permanent — important for influencer reference photos) and falls back to
// fal storage, whose URLs proved to expire. Throws if neither works.
export async function uploadDataUrl(dataUrl, prefix = "upload") {
  ...
  // 1) Vercel Blob — permanent public URLs (fal storage URLs expire).
  if (configured()) {
    try {
      ...
      if (!(process.env.FAL_KEY || process.env.FAL_API_KEY)) throw e; // no fal fallback available
    }
  }

  // 2) fal storage fallback (only needs FAL_KEY).
  if (!(process.env.FAL_KEY || process.env.FAL_API_KEY)) throw new Error("No file host configured (set BLOB_READ_WRITE_TOKEN or FAL_KEY)");
