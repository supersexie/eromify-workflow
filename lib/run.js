...
async function generateImage({ prompt, model, images, aspect, quality }) {
  ...
  // OpenAI (GPT Image) refuses/stalls on edits of real people, so a reference +
  // GPT model that never returns is almost always a silent moderation block.
  const hasRef = Array.isArray(images) && images.length > 0;
  if (hasRef && /^GPT Image/.test(model || "")) {
