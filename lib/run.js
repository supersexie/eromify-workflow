export async function generateOutput(kind, prompt, model, images, opts = {}) {
  ...
  // A moderation block/review returns an `error` (403/202); surface it rather
  // than silently returning an undefined output.
...
export async function combineVideos(urls, durations, onProgress) {
  ...
  // Note: a moderation "review" verdict returns HTTP 202 (a success code, so
  // res.ok is true) with an `error` message and no handle — treat any response
  // carrying an `error` as a thrown error so the message surfaces instead of
  // falling through to a confusing "Missing fal handle".
...
export async function generateVideo({ prompt, model, image, aspect, resolution, duration, audio }, onProgress) {
  const startRes = await fetch("/api/video/start", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ prompt, model, image, aspect, resolution, duration, audio }),
  });
  const start = await startRes.json();
  // Note: a moderation "review" verdict returns HTTP 202 (a success code, so
  // res.ok is true) with an `error` message and no handle — treat any response
