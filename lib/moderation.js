...
export async function screenPromptHive(prompt) {
  ...
  try {
    ...
    // Hard blocks — minor safety first.
    if (childExploit >= 1) return { verdict: "block", reason: "text_child_exploitation", severities };
    if ((minorExplicit >= 1 || minorImplicit >= 1) && sexual >= 1) {
      return { verdict: "block", reason: "text_minor_with_sexual", severities };
    }
    if (childSafety >= 2) return { verdict: "block", reason: "text_child_safety", severities };
    ...
    if (minorExplicit >= 2 || minorImplicit >= 2 || childSafety >= 1) return { verdict: "review", reason: "text_possible_minor", severities };
