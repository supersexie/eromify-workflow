...
export async function classifyOutput(mediaUrl) {
  ...
  try {
    const { scoreOf } = await hiveClassify({ url: mediaUrl });
    ...
    if (csamScore > 0.15 || suggestiveMinorScore > 0.3) return { verdict: "block", scores };
    if (suggestiveMinorScore > 0.05 || csamScore > 0.02) return { verdict: "review", scores };
    return { verdict: "allow", scores };
  } catch (e) {
    // Fail closed — moderation service down means we do not serve content.
    return { verdict: "block", reason: "moderation_service_unavailable", scores: {} };
  }
}
