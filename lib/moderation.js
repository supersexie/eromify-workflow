...
// ─── review queue ───────────────────────────────────────────────────────────
// Default: console-only (0 Blob Advanced ops). Set MODERATION_BLOB_QUEUE=1 to
// also persist to moderation-queue.json for human review.
...
/**
 * Logs a block/review verdict for human follow-up. Best-effort — never throws.
 */
export async function queueForReview(record) {
  ...
  // Skip Blob by default — list+put on every block was burning Advanced ops.
  if (process.env.MODERATION_BLOB_QUEUE !== "1") return entry;
  ...
  try {
    let existing = [];
    try {
      const result = await get(QUEUE_INDEX_PATH, { access: "public", token, useCache: false });
      if (result?.stream) {
        const chunks = [];
