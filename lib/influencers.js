...
// localStorage stays the fast SYNC cache; the server is durable. Deduped so
// ImagePage + MentionField mounting together only hit Blob once.
...
// local influencers, migrate them up. Returns the resulting list.
export async function syncInfluencers({ force = false } = {}) {
  if (typeof window === "undefined") return [];
  if (!force && Date.now() - syncAt < SYNC_MIN_MS) return listInfluencers();
...
// Appended to a prompt when an influencer reference is attached. Nano Banana
// Pro (Gemini) in particular renders a generic person unless told to preserve
// the reference's identity; harmless reinforcement for the other edit models.
