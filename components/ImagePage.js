...
export default function ImagePage() {
  ...
  const generate = async () => {
    ...
    if (mode === "swap") {
      ...
      finalPrompt = `Photorealistic face swap. The FIRST image is the final photo — keep its exact composition, framing, background, body, pose, hair, clothing, camera angle, and lighting unchanged. Transfer ONLY the facial identity of the person in the SECOND image: their face shape, bone structure, eyes, nose, lips, eyebrows, jawline, and distinctive features, so the result is clearly recognizable as that person.\n\nCritically, do NOT copy the second image's head pose, gaze, expression, or lighting. Conform the new face to the FIRST image: match its exact head orientation and tilt, the gaze/eye direction, the facial expression, and the camera perspective. Relight the face to the first image's light direction, intensity, soft/hard shadows, and color temperature; seamlessly blend skin tone, contrast, and color grade with the neck and body already in the first image. Match the first image's photographic qualities — lens, depth of field, sharpness, grain, and white balance — so it reads as one real, uncomposited photograph with no seams around the hairline or jaw.${extra ? ` Additional guidance: ${extra}.` : ""}`;
