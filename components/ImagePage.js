  ...
  const generate = async () => {
    ...
    if (mode === "swap") {
      ...
      if (model === "Nano Banana Pro") {
        ...
        finalPrompt = `Take the scene, pose, body, clothing, lighting, and composition from the first image. Replace ONLY the person's face and hair with the person shown in the second image — keep their exact face, facial features, and hair identical to the second image. Everything else (background, clothing, pose, framing) must stay identical to the first image. Match the skin tone of the neck, shoulders, arms, hands, and all visible body skin to the new face's complexion so the skin color is consistent across the whole body with no mismatch at the neck or jawline.${extra ? ` Additional guidance: ${extra}.` : ""}`;
      } else {
      ...
      <div className="ip-bar">
        <div className="ip-bar-inner">
          ...
          <div className="ip-bar-input-row">
            ...
            <button className="pb-enhance" onClick={enhancePrompt} disabled={enhancing || !prompt.trim()} title="Enhance prompt with Magic Mint style">
              {enhancing ? (
