...
export async function POST(req) {
  ...
  try {
    ...
    if (s.status === "IN_QUEUE" || s.status === "IN_PROGRESS") return NextResponse.json({ done: false });
    ...
    // Image upscalers return image/images; video upscalers return video.
    const url =
      result.image?.url ||
      result.images?.[0]?.url ||
      result.video?.url ||
      result.output?.url;
    if (!url) throw new Error("No upscaled media URL in fal result");
